...
def create_draft_survey(payload: dict, conn=Depends(get_db)):
    ...
    try:
        ...
        def _clean_value(v):
            ...
            if isinstance(v, str):
                vv = v.strip()
                if vv == "" or vv.lower() in ("none", "null"):
                    return None
                return vv
            return v
        ...
        # =====================================================
        # 🔥 NORMALIZE NUMERIC (SOLO SI APLICA)
        # =====================================================
        def safe_numeric(v):
            try:
        ...
        for k, v in payload.items():
            payload[k] = safe_numeric(v)

        # =====================================================
        # 🔥 NORMALIZE TEXT (NO PEGAR PALABRAS)
        # =====================================================
        ...
        text_fields = [
            "vessel_mv", "client", "country", "port",
            "master", "chief_officer", "survey_requested_by"
        ]

        for key in text_fields:
            if payload.get(key):
                payload[key] = normalize_text(payload[key])
        ...
        payload["init_time_from"] = normalize_time(payload.get("init_time_from"))
        payload["init_time_to"] = normalize_time(payload.get("init_time_to"))
        payload["final_time_from"] = normalize_time(payload.get("final_time_from"))
        payload["final_time_to"] = normalize_time(payload.get("final_time_to"))
