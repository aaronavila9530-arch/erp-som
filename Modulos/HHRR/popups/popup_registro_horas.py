...
from api_client import hr_create_ot_log


class PopupRegistroHoras(tk.Toplevel):
    """
    Popup para REGISTRO DE HORAS (OT LOG)

    ✔ Calendario real (DateEntry)
    ✔ Reloj visible (Spinbox clásico)
    ✔ ISO datetime válido
    ✔ Backend calcula duración
    """
    ...
    def _guardar(self):
        ...
        try:
            # 🔑 CONSTRUIR DATETIME REAL (NO STRINGS)
            inicio_dt = datetime.combine(
                self.cal_inicio.get_date(),
                datetime.min.time()
            ).replace(
            ...
            payload = {
                ...
                "fecha_inicio": inicio_dt.isoformat(sep=" "),
                "fecha_fin": fin_dt.isoformat(sep=" "),
                "buque": self.ent_buque.get() or None,
                "comentario": self.txt_comentario.get("1.0", "end").strip() or None
            }

            hr_create_ot_log(payload)
