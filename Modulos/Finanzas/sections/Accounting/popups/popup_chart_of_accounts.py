...
class PopupChartOfAccounts(tk.Toplevel):
    ...
    def _build_ui(self):
        ...
        columns = ("code", "name", "type", "nature", "level", "posting", "third", "cost", "active")
        self.tree = ttk.Treeview(self, columns=columns, show="headings", height=18)
        headers = ("Código", "Nombre", "Tipo", "Naturaleza", "Nivel", "Movimientos", "Tercero", "Centro costo", "Activa")
    ...
    def _load(self):
        ...
        for account in get_accounting_accounts_api():
    ...
    def _select(self, _event=None):
        ...
        account = next((item for item in get_accounting_accounts_api() if item.get("account_code") == self.selected_code), None)
        if not account:
            return
