...
def export_diario_excel(rows: list[dict], fiscal_year: int, period: int):
    ...
    for r in rows:
        ...
        ws.cell(row=row, column=5, value=r.get("account_type", ""))

        debe = float(r.get("debit") or 0)
        haber = float(r.get("credit") or 0)

        ws.cell(row=row, column=6, value=debe)
        ws.cell(row=row, column=7, value=haber)
        ws.cell(row=row, column=8, value=r.get("line_description", ""))
        ws.cell(row=row, column=9, value=r.get("origin", ""))
    ...
    ws.cell(row=row, column=6, value=total_debe).font = Font(bold=True)
    ws.cell(row=row, column=7, value=total_haber).font = Font(bold=True)
